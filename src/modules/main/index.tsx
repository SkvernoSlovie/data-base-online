...
enum ColumnTitles {
  name = 'Название',
  product = 'Изделие',
  measurableValue = 'Измеряемая величина',
  type = 'Тип устройства',
...
const Main: React.FC = () => {
  ...
  const [deviceLoading, sensorLoading] = useFetchData();
  const [removeSensor] = mainApi.useRemoveSensorMutation();
  const [removeDevice] = mainApi.useRemoveDeviceMutation();
  const sensors = useAppSelector((state) => state.mainReducers.sensors);
  const devices = useAppSelector((state) => state.mainReducers.devices);
        ...
        {isSensorTableActive ? (
          <Table
            ...
            data={filterSensorData.map((item) => ({
              id: item.id,
              product: 'Датчик',
              measurableValue: item.measurable_values[0]?.name,
              name: item.name,
              type: item.type[0]?.name,
              measuringRange: `${item.measure_min} - ${item.measure_max} ${item.unit_of_measuring}`,
              resource: `${item.resource} ч.`,
